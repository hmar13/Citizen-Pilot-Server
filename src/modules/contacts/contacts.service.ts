...
export class ContactsService {
  constructor(@Inject(CONTACT_REPOSITORY) private readonly contactsRepository: typeof Contact) { }

  async create(contacts: ContactsDto, userId): Promise<Contact> {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    return await this.contactsRepository.create<Contact>({ ...contacts, userId})
  }
  ...
  async update(id, data, userId) {
    const [numberOfAffectedRows, [updatedContact]] = await this.contactsRepository.update(
